...
/*@NamedNativeQueries({
    @NamedNativeQuery(
            name = "Catalogos.GetUensAndLocsBySupplier",
            query = "select uens.organization_id, organization_name, locs.id_localizacion, locs.nombre_localizacion  from (select distinct o.organization_id, o.organization_name\n"
                   + "   from nvc_vm_oa_proveedores_sites p\n"
                   + "    join oa_uens o on o.organization_id = p.id_uen\n"
                   + "    where id_proveedor = 9636\n"
                   + "    order by o.organization_name asc) uens\n"
                   + "    inner join (select a.id_uen, a.id_localizacion, b.nombre_localizacion --|| ' - ' || a.direccion nombre_localizacion\n"
                   + "        from localizaciones_por_uen_cia a, nvc_vm_oa_localizaciones b\n"
                   + "        where a.id_localizacion = b.id_localizacion and a.estatus = 1\n"
                   + "        order by b.nombre_localizacion ) locs\n"
                   + "    on uens.organization_id = locs.id_uen\n"
                   + "    order by uens.organization_id asc\n",
            resultSetMapping = "Catalogo.UensWithLocs"
    )
})*/
...
@NamedStoredProcedureQueries({
    ...
    /*@NamedStoredProcedureQuery(
                name = "Catalogos.UensAndLocsBySupplier", 
                procedureName = "pkg_catalogo_sp.getUensAndLocsBySupplier",
                resultSetMappings = "Catalogo.UensWithLocs",
                parameters = {
                    @StoredProcedureParameter(name = "p_id_proveedor", type = String.class, mode = ParameterMode.IN),
                    @StoredProcedureParameter(name = "out_cursor", type = void.class, mode = ParameterMode.REF_CURSOR)
                }
    ...
    @NamedStoredProcedureQuery(
        ...
        @NamedStoredProcedureQuery(
                ...
                //resultSetMappings = "Catalogo.savedCatalogo",
                parameters = {
